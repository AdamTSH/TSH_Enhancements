...
(function() 
{
    ...
    for(var t = 0; t < torrentArray.length; t++)
    {
        ...
        if(torrentArray[t].getElementsByClassName('cats_tvhd').length == 1)
        {
            ...
            var match = torrentName.innerHTML.match(/(.*)\.S(\d\d)E(\d\d).*(720p|1080i|1080p)\.(HDTV|WEB-DL|WEBRiP|WEBRip).*(H.264|x264|h264)-(.*)/);
            if (match !== null) 
            {
            ...
            var match = torrentName.innerHTML.match(/(.*)\.(\d\d\d\d)\.(\d\d)\.(\d\d).*(720p|1080i|1080p)\.(HDTV|WEB-DL|WEBRiP|WEBRip).*(H.264|x264|h264)-(.*)/);
            if (match !== null) 
            {
                var detailName = match[1].replace(/\./g, " ");
        ...
        if(torrentArray[t].getElementsByClassName('cats_tvsd').length == 1)
        {
            ...
            var match = torrentName.innerHTML.match(/(.*)\.S(\d\d)E(\d\d)\.(HDTV|WEB-DL|WEBRiP|WEBRip).*(H.264|x264|h264)-(.*)/);
            if (match !== null) 
            {
                var detailName = match[1].replace(/\./g, " ");
                var detailSeason = match[2];
            ...
            var match = torrentName.innerHTML.match(/(.*)\.(\d\d\d\d)\.(\d\d)\.(\d\d)\.(HDTV|WEB-DL|WEBRiP|WEBRip).*(H.264|x264|h264)-(.*)/);
            if (match !== null) 
            {
                var detailName = match[1].replace(/\./g, " ");
                var detailDateYear = match[2];
